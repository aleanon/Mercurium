...
impl Ed25519KeyPair {
    ...
    pub fn new(index: u32, network: Network, entity: Bip32Entity, key_kind: Bip32KeyKind) -> (Self, [u32; 6]) {
    ...
    pub fn from_mnemonic(mnemonic: &Mnemonic, index: u32, network: Network, entity: Bip32Entity, key_kind: Bip32KeyKind) -> (Self, [u32;6]) {
        ...
        //The starting "m/" is omitted from the derivation path with this implemetation
        let derivation_path = [
            BIP32_LEAD_WORD,
        ...
        (Self {secret_key, public_key, network, entity, key_kind}, derivation_path)
    }
    ...
    pub fn bech32_address (&self) -> String {
        ...
        let virtual_account_address = ComponentAddress::virtual_account_from_public_key(&self.radixdlt_public_key());
        ...
        let address = encoder.encode(virtual_account_address.as_ref()).expect("Unreachable error, invalid Bech32 address");
...
mod test {
    ...
    fn test_address_from_mnemonic_with_index_mainnet() {
        ...
        let (keypair, _) = Ed25519KeyPair::from_mnemonic(&mnemonic, 0, Network::Mainnet, Bip32Entity::Account, Bip32KeyKind::TransactionSigning);
        let account_address = keypair.bech32_address();
        println!("account_address: {}", account_address);
        let (keypair2, _) = Ed25519KeyPair::from_mnemonic(&mnemonic, 1, Network::Mainnet, Bip32Entity::Account, Bip32KeyKind::TransactionSigning);
        let account_address2 = keypair2.bech32_address(); 
        println!("account_address2: {}", account_address2);

        assert_eq!(account_address.as_str(), "account_rdx128ykx9agh0maq8nw6h6pzmltmaexts0xf24sledqp44x5cdec0uqjj");
        assert_eq!(account_address2.as_str(), "account_rdx12xn8d9ykr8pmch33v0q66vhpvt98afalg0tfgctrqenkly96sgdx5n");
    }
    ...
    fn test_address_from_mnemonic_with_index_stokenet() {
        ...
        let (keypair, _) = Ed25519KeyPair::from_mnemonic(&mnemonic, 0, Network::Stokenet, Bip32Entity::Account, Bip32KeyKind::TransactionSigning);
        let account_address = keypair.bech32_address();
        println!("account_address: {}", account_address);
        let (keypair2, _) = Ed25519KeyPair::from_mnemonic(&mnemonic, 1, Network::Stokenet, Bip32Entity::Account, Bip32KeyKind::TransactionSigning);
        let account_address2 = keypair2.bech32_address(); 
        println!("account_address2: {}", account_address2);

        assert_eq!(account_address.as_str(), "account_tdx_2_12y0kpp2nhn8f36gt2ppqmxeltj6n2r446s0jlh4l7yxttpfeahjn66");
        assert_eq!(account_address2.as_str(), "account_tdx_2_12866llg04px7q2wee02yxcxtdwgtpzdc8n75fermd070u64t98jtnj");
    }
}