...
use data_stores::ports::wallet_data_store::WalletDataStore;

use crate::port::{AssetSummary, LedgerConnector, Resource, ResourceId};

type Account = types::Account;
type AccountId = types::address::AccountAddress;
type Network = types::Network;
type AccountsUpdate = types::collections::AccountsUpdate;
type AccountUpdate = types::collections::AccountUpdate;

pub struct RadixOfficialGateway {
    pub network: Network,
}

impl RadixOfficialGateway {
    ...
    pub async fn update_all_accounts(
        &self,
        db: impl WalletDataStore,
    ) -> Result<AccountsUpdate, AppError> {
        let db = AppDataDb::get(self.network)
    ...
    async fn update_account(
        ...
        let (
            new_state_version_non_fungible_balances,
            non_fungible_assets,
            new_non_fungible_resources,
...
impl LedgerConnector for RadixOfficialGateway {
    ...
    async fn update_account(
        &self,
        account: Account,
        resources: impl IntoIterator<Item = (ResourceId, Resource)>,
    ) -> AccountUpdate {
        todo!()
    }
