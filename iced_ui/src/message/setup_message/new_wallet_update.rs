...
use crate::{
        app::{App, AppData, AppState}, message::{common_message::CommonMessage, login_message::LoginMessage, Message}, view::setup::{new_wallet::{self, NewWalletStage}, Setup} 
    };
use store::Db;
...
impl<'a> WalletMessage {
    pub fn process(self, setup:&'a mut Setup, app_data:&'a mut AppData) -> Command<Message> {
        if let Setup::NewWallet(new_wallet_state)= setup {
            match self {
                ...
                Self::UpdatePassword(mut input) => Self::update_password_input(&mut input, new_wallet_state),
                Self::SubmitPassword => Self::submit_password(new_wallet_state),
                Self::UpdateVerificationPassword(input) => Self::update_verified_password_input(input, new_wallet_state), 
                Self::VerifiPassword => Self::verifi_password(new_wallet_state),
                Self::UpdateAccName(input) => Self::update_account_name_input(input, new_wallet_state), 
                ...
                Self::UpdateInputSeed((index, words)) => Self::update_input_seed(index, words, new_wallet_state),
    ...
    fn update_password_input(input: &mut String, new_wallet_state: &'a mut NewWallet) -> Command<Message> {
        new_wallet_state.password.clear();
    ...
    fn update_verified_password_input(mut input: String, new_wallet_state: &'a mut NewWallet) -> Command<Message> {
        new_wallet_state.verify_password.clear();
    ...
    fn update_account_name_input(input: String, new_wallet_state: &'a mut NewWallet) -> Command<Message> {
    ...
    fn update_input_seed(mut index: usize, words: Vec<String>, new_wallet_state: &'a mut NewWallet) -> Command<Message> {
    ...
    fn create_wallet(new_wallet_state: &'a mut NewWallet, app_data: &'a mut AppData) -> Command<Message> {
