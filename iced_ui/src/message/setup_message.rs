...
use crate::{
    app::AppData,
    view::setup::{
        new_wallet::{NewWallet, NewWalletStage},
        Setup,
    },
};
...
impl<'a> SetupMessage {
    pub fn process(self, setup: &'a mut Setup, app_data: &'a mut AppData) -> Command<Message> {
        ...
        match self {
            ...
            Self::FromSeed => *setup = Setup::NewWallet(NewWallet::new_without_mnemonic()),
    ...
    fn back(setup: &'a mut Setup) {
        match setup {
            Setup::NewWallet(new_wallet_state) => match new_wallet_state.stage {
                NewWalletStage::EnterPassword => *setup = Setup::SelectCreation,
