...
use types::{
    ...
    crypto::Key,
    crypto::{
        EncryptedMnemonic, HashedPassword, KeySaltPair, Password,
        bip39::{Language, Mnemonic, MnemonicType},
    },
};

use crate::{
    Unlocked, Wallet, WalletData,
    settings::Settings,
    wallet::{WalletState, create_account_from_mnemonic, resource_data::ResourceData},
...
impl Setup {
    ...
    pub fn set_password(&mut self, new_password: Password) {
        ...
        let (password, id) = self
            .password
            .as_ref()
            .unwrap_unreachable("Called unwrap on password, but no password was supplied");
        ...
        tokio::spawn(async move {
            task_manager
                .run_task_create_encryption_keys(id, password)
                .await;
        });
    }

    pub fn set_mnemonic_and_password(
        &mut self,
        new_mnemonic: Mnemonic,
        new_seed_password: Option<Password>,
    ) {
        match &mut self.mnemonic_with_password {
            Some((mnemonic, seed_password, id)) => {
                if new_mnemonic.phrase() == mnemonic.phrase()
                    && new_seed_password.as_ref() == seed_password.as_ref()
                {
                    return;
                }
        ...
        let (mnemonic, seed_password, id) = self
            .mnemonic_with_password
            .as_ref()
        ...
        tokio::spawn(async move {
            task_manager
                .run_task_create_and_update_accounts(id, mnemonic, seed_password, network)
                .await;
        });
    }
    ...
    pub fn get_seed_password(&self) -> Option<&str> {
        self.mnemonic_with_password
            .as_ref()
            .and_then(|(_, pw, _)| pw.as_ref().map(|pw| pw.as_str()))
    }
    ...
    pub async fn finalize_setup(mut self) -> Result<Wallet<Unlocked>, SetupError> {
        ...
        debug_println!(
            "Saved {} icons to disk",
            wallet_data.resource_data.resource_icons.len()
        );
...
fn save_updated_accounts_to_resource_data(
    ...
    mut accounts_update: AccountsUpdate,
    ...
    for mut account in accounts {
        ...
        resource_data
            .accounts
            .insert(updated_account.address, account);
    }
